...
function buildRisks(): ExportRunResult['risks'] {
  return [
    ...
    },
    {
      ...
      level: 'info',
      message: 'scenario-detail-pages are deferred in 22-6-4; current JSON source is empty and outside the main export registry.',
    },
    {
    ...
    },
    {
      ...
      level: 'blocker',
      message: '--write is disabled in 22-6-4 and must not overwrite server/data.',
    },
    {
    ...
    },
    {
      ...
      level: 'info',
      message: 'No real API write tests are part of the 22-6-4 dry-run export.',
    },
    {
...
export async function runExportDryRun(options: ExportCliOptions): Promise<ExportRunResult> {
  if (options.writeRequested) {
    throw new Error('--write is not supported in 22-6-4. This dry-run export never overwrites server/data.');
  }
  ...
  const manifest: ExportManifest = {
    ...
    canRollback: false,
    warnings: [
      '22-6-4 is still dry-run only; implemented module exports are report artifacts, not official server/data writes.',
